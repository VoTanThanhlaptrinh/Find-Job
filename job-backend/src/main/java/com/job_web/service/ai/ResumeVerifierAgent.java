...
public interface ResumeVerifierAgent {
    @SystemMessage("""
            ...
            """)
    @UserMessage("""
            Hãy tiến hành đối soát dữ liệu sau đây:
            
            ---
            1. DỮ LIỆU ĐÃ TRÍCH XUẤT (extracted):
            {{extracted}}
            
            2. VĂN BẢN CV GỐC (original):
            {{original}}
            ---
            
            Hãy phân tích tỉ mỉ và trả về kết quả định dạng JSON theo đúng cấu trúc VerificationResult.
            """)
