...
public class ApplyServiceImpl implements ApplyService {
    ...
    public ApiResponse<String> applyWithExistingCv(ApplyCvWithExistingRequest request, User user) {
        ...
        if(user.getApplies().stream().anyMatch(apply -> apply.getJob().getId() == request.getJobId())){
            return new ApiResponse<>("Người dùng đã apply", null, HttpStatus.BAD_REQUEST.value());
        }
        if(!resume.get().getUser().getEmail().equals(user.getEmail())){
    ...
    public ApiResponse<String> applyWithUploadCv(ApplyCvWithUploadRequest request, User user) throws IOException {
        ...
        try{
            ...
            messageProducer.processAI(new ResumeParsingMessage(rawText, user.getId(), resume.getId()));
